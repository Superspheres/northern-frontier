...
public sealed class SharedFirearmAttachmentSystem : EntitySystem
{
    ...
    {
        RefreshGunIfMuzzleChanged(ent, args.Container.ID);
    ...
    private void RefreshGunIfMuzzleChanged(Entity<FirearmAttachmentHostComponent> ent, string containerId)
    {
        if (containerId != ent.Comp.MuzzleSlotId)
            return;
